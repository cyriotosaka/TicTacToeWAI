...
public class Board {
     ...
     */
    public StateO stepGame(SeedO player, int selectedRow, int selectedCol) {
        // Update game board
        ...
        if (cells[selectedRow][0].content == player  // 3-in-the-row
            ...
            return (player == SeedO.CROSS) ? StateO.CROSS_WON : StateO.NOUGHT_WON;
        } else {
            // Nobody win. Check for DRAW (all cells occupied) or PLAYING.
            for (int row = 0; row < ROWS; ++row) {
                for (int col = 0; col < COLS; ++col) {
                    if (cells[row][col].content == SeedO.NO_SEED) {
                        return StateO.PLAYING; // still have empty cells
                    }
                }
            }
            return StateO.DRAW; // no empty cell, it's a draw
        }
    }
