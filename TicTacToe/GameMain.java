...
public class GameMain extends JPanel {
    ...
    private StateO currentState;  // the current state of the game
    private SeedO currentPlayer;  // the current player
    ...
    public void newGame() {
        for (int row = 0; row < Board.ROWS; ++row) {
            for (int col = 0; col < Board.COLS; ++col) {
                board.cells[row][col].content = SeedO.NO_SEED; // all cells empty
            }
        }
        currentPlayer = SeedO.CROSS;    // cross plays first
        currentState = StateO.PLAYING;  // ready to play
    }
